...
describe('RecipeDetail - Edit Functionality', () => {
  ...
  it('should display editable content textarea for manual recipes', async () => {
    ...
    const contentTextarea = screen.getByPlaceholderText('Enter recipe content...');
  ...
  it('should validate required content for manual recipes', async () => {
    ...
    // Should show validation error
    expect(screen.getByText('Content is required for manual recipes')).toBeInTheDocument();
  ...
  it('should display loading state while saving', async () => {
    ...
    api.recipesApi.getById.mockResolvedValue(mockRecipe);
    api.recipesApi.update.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
  ...
  it('should hide favorite button in edit mode', async () => {
    ...
    const favButton = favButtons.find(btn => btn.querySelector('svg path[d*="M11.049"]'));
    ...
    const favButtonAfter = favButtonsAfter.find(btn => btn.querySelector('svg path[d*="M11.049"]'));
